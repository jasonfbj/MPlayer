...
public:
    ...
    virtual bool SetRulesColour(const wxColour& colour) wxOVERRIDE;

    virtual bool SetBorderColour(const wxColour& colour) wxOVERRIDE
    {
    ...
    // The method is only avalibale in the generic versions.
    wxWindow* GenericGetClient() const;
...
private:
    ...
    // user defined color to draw first line, may be invalid
    // used with wxDV_FIRST_LINE_AS_HEADER
    wxColour m_headerColour;
    wxColour m_headerTextColour;
    wxFont   m_headerFont;

    // user defind border colour
