...
public:
    ...
    bool HasFont() const { return m_bold || m_italic || m_strikethrough || m_font.IsOk();  }
...
#define wxDV_FIRST_ROW_AS_HEADER     0x0040     // the first row as header
...
public:
    ...
    virtual wxDataViewItem GetLastItem() const {return wxDataViewItem(NULL);}
    ...
    // subclass can return false to prevent high-light current item.
    virtual bool HighlightCurrentItem() const { return true; }

    virtual void DrawBackgroundCustom(wxDC& WXUNUSED(dc), const wxRect& WXUNUSED(rowRect))
    {
        // do nothing
    }

    virtual bool CustomDrawBackground(int WXUNUSED(item)) const
    {
        return false;
    }

    inline virtual void EnableAutoScroll(bool enable) 
    {
        m_autoScroll = enable;
    }

    virtual bool AutoScroll() const
    {
        return m_autoScroll;
    }

    virtual bool IsCustomScrollRate() const
    {
        return false;
    }

    virtual bool DrawFocusBackground() const { return true; }
    virtual bool DrawSelectionBackgroud() const { return true; }
    ...
    virtual bool SetRulesColour(const wxColour& WXUNUSED(colour))
    {
        return false;
    }

    virtual bool SetBorderColour(const wxColour& WXUNUSED(colour))
    {
        return false;
    }
